...
function VideoCall({ myId, otherId }) {
    ...
    if (typeof window !== "undefined" && myId && otherId) {
      ...
      peer.on("call", (call) => {
        ...
        if (
          ...
          peerInstance.current._connections[otherId]
        ) {
          // todo: might not work
        ...
        navigator.mediaDevices
          ...
          .then((mediaStream) => {
            // get video and audio stream from device, stream contains mult tracks
  ...
  return (
    <div className="flex-col p-4">
      ...
      {!isVideoLoaded ? null : (
        <div className="flex justify-between pb-2 mt-4">{otherId}</div>
      )}
      <video
        playsInline
        ref={remoteVideoRef}
