...
const authenticateSocket = (socket, next) => {
  ...
  if (!token) {
    return next(new Error('Authentication failed in matching-service: Token missing'));
  }

  jwt.verify(token.replace("Bearer ", ""), KEY, (err, decoded) => {
    if (err) {
      return next(new Error('Authentication failed in matching-service: Invalid token'));
    }
